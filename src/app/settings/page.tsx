  ...
  return (
    <div className="space-y-4">
      ...
      <div className="vintage-card p-4" style={{ display: 'flex', gap: '8px' }}>
        <button className="vintage-button" onClick={onExport}>
          JSON 내보내기
        </button>
        <input
        ...
        />
        <button className="vintage-button" onClick={sampleUpsert} disabled={!preview}>
          샘플 업서트 5개
        </button>
      </div>
      {msg ? <div className="vintage-card p-3">{msg}</div> : null}
      {json ? (
        <pre className="vintage-card p-4" style={{ maxHeight: '320px', overflow: 'auto' }}>
          {json}
        </pre>
      ) : null}
      {preview ? (
        <div className="vintage-card p-4">
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              <strong>Books</strong>
              <pre>{JSON.stringify(preview.books, null, 2)}</pre>
            </div>
            <div>
              <strong>Notes</strong>
              <pre>{JSON.stringify(preview.notes, null, 2)}</pre>
            </div>
            <div>
              <strong>Tags</strong>
              <pre>{JSON.stringify(preview.tags, null, 2)}</pre>
            </div>
            <div>
              <strong>Entities</strong>
              <pre>{JSON.stringify(preview.entities, null, 2)}</pre>
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <strong>Links</strong>
              <pre>{JSON.stringify(preview.links, null, 2)}</pre>
            </div>
          </div>
        </div>
      ) : null}
