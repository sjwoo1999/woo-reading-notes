...
async function createNote(bookId: string, formData: FormData) {
  ...
  const res = await fetch(`${base}/rest/v1/notes`, {
    method: 'POST',
    headers: {
      apikey: key,
...
async function deleteBook(bookId: string) {
  ...
  await fetch(
    `${base}/rest/v1/links?or=(and(src_type.eq.book,src_id.eq.${bookId}),and(dst_type.eq.book,dst_id.eq.${bookId}))`,
    {
      method: 'DELETE',
      headers: { apikey: key, Authorization: `Bearer ${key}` },
    }
  );
  await fetch(`${base}/rest/v1/books?id=eq.${bookId}`, {
    method: 'DELETE',
    headers: { apikey: key, Authorization: `Bearer ${key}` },
  });
  redirect('/library');
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div>
          ...
          {book.summary ? (
            <p className="text-sm mt-2 opacity-90 whitespace-pre-wrap">{book.summary}</p>
          ) : null}
        </div>
      </div>

      <form
        action={deleteBook.bind(null, id)}
        className="vintage-card p-4"
        style={{ display: 'flex', justifyContent: 'flex-end' }}
      >
        <button className="vintage-button--danger" aria-label="도서 삭제">
          도서 삭제
        </button>
      </form>

      <form
        action={createNote.bind(null, id)}
        className="vintage-card p-4"
      ...
      <div className="grid gap-2">
        {notes.map((n: any) => (
          <div key={n.id} className="vintage-card p-3">
            ...
            {n.content ? (
              <div className="text-sm opacity-80 mt-1 whitespace-pre-wrap">{n.content}</div>
            ) : null}
            {n.updated_at ? (
