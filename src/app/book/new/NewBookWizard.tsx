...
export default function NewBookWizard() {
  ...
  async function search(p = page, s = size) {
    ...
    try {
      const res = await fetch(
        `/api/books?${new URLSearchParams({ query: q, page: String(p), size: String(s) }).toString()}`
      );
  ...
  async function submit() {
    ...
    try {
      ...
      const publishedYear =
        form.publishedDate && /^(\d{4})/.test(form.publishedDate)
          ? Number(form.publishedDate.slice(0, 4))
          : null;
      // books insert
      ...
      const bRes = await fetch(`${base}/rest/v1/books`, {
        method: 'POST',
        headers: {
          apikey: key,
      ...
      if (form.note && form.note.trim()) {
        await fetch(`${base}/rest/v1/notes`, {
          ...
          body: JSON.stringify({ book_id: bookId, title: '메모', content: form.note }),
        });
      }
      ...
      for (const name of tags) {
        ...
        const getRes = await fetch(
          `${base}/rest/v1/tags?select=id,name&name=eq.${encodeURIComponent(name)}`,
        ...
        if (!tagId) {
          const ins = await fetch(`${base}/rest/v1/tags`, {
            method: 'POST',
            headers: {
              apikey: key,
              Authorization: `Bearer ${accessToken}`,
        ...
        if (tagId) {
          await fetch(`${base}/rest/v1/book_tags`, {
            method: 'POST',
            headers: {
              apikey: key,
              Authorization: `Bearer ${accessToken}`,
            ...
            body: JSON.stringify({ book_id: bookId, tag_id: tagId }),
          });
        }
      }
        ...
        <div className="space-y-3">
          ...
          <div className="vintage-card p-4">
            <table className="text-sm w-full">
              <thead>
                <tr>
                  <th className="text-left">표지</th>
                  <th className="text-left">제목</th>
                  <th className="text-left">저자</th>
                  <th className="text-left">출판사</th>
                  <th className="text-left">출간일</th>
                  <th></th>
                </tr>
              </thead>
              <tbody>
                {loading && results.length === 0 ? (
                  ...
                  <tr>
                    <td colSpan={6}>
                      <div className="text-sm opacity-70">검색 결과가 없습니다</div>
                    </td>
                  </tr>
                ) : (
                  results.map((b) => (
                    <tr key={b.isbn}>
                      ...
                      <td>{b.publishedAt}</td>
                      <td>
                        <button className="vintage-button" onClick={() => onPick(b)}>
                          선택
                        </button>
                      </td>
                    </tr>
        ...
        <div className="space-y-3">
            ...
            <div className="h-stack" style={{ gap: 12, alignItems: 'flex-start' }}>
              <div className="cover-7-10" style={{ width: 160 }}>
                {pick?.thumbnail ? (
                  ...
                  <div
                    aria-hidden="true"
                    style={{ width: '100%', height: '100%', background: 'var(--paper, #f9f7f2)' }}
                  />
                )}
              </div>
              <div className="v-stack" style={{ gap: 4 }}>
                <div className="text-sm">
                  {pick?.title} — {(pick?.authors || []).join(', ')} / {pick?.publisher}
                </div>
          ...
          <div className="vintage-card p-4 v-stack">
            ...
            <div className="h-stack">
              <label className="v-stack" style={{ flex: 1 }}>
                출간일
                <input
                  className="vintage-input"
                  type="date"
                ...
                <div className="h-stack" role="radiogroup" aria-label="평점">
                  {[0, 1, 2, 3, 4, 5].map((n) => (
                    <button
                      ...
                      style={{
                        padding: '4px 8px',
                        background: form.rating === n ? '#3B4E76' : 'transparent',
                        color: form.rating === n ? '#fff' : 'inherit',
                      }}
                    >
                      {'★'.repeat(n)}
                      {'☆'.repeat(5 - n)}
                ...
                {(form.tagsText || '')
                  ...
                  .map((t) => (
                    <span
                      ...
                      style={{ padding: '2px 8px', borderRadius: 12, fontSize: 12 }}
                    >
                      {t}
                    </span>
                  ))}
            ...
            <div className="h-stack" style={{ justifyContent: 'space-between' }}>
              <button className="vintage-button" onClick={() => setStep(1)}>
                이전으로
              </button>
              <button className="vintage-button" onClick={submit}>
                등록
              </button>
            </div>
